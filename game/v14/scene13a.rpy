# SCENE 13a: Apes Jetlag Sleep
# Locations: MC Ape Room
# Characters: MC (Outfit: 5), GRAYSON (Outfit: 3)
# Time: Evening

label v14s13a:
    scene v14s13a_1 # TPP. Show MC walking into his room, carrying his luggage, smiling, mouth closed
    with fade

    u "(Look at that beautiful bed... Oh, the things I'm gonna do to you.) *Chuckles*"

    scene v14s13a_2 # TPP. Show MC inside his room, closing the door (door not fully closed yet), smiling, mouth closed
    with dissolve

    u "(I won't be letting anyone bother me tonight.)"

    scene v14s13a_3 # TPP. Close up of Grayson's foot blocking the door from closing
    with dissolve

    pause 0.75

    scene v14s13a_4 # FPP. MC standing in front of door, door now open, Grayson on the other side, Grayson smiling, mouth open
    with dissolve

    gr "Charli's boyfriend is back!"

    scene v14s13a_4a # FPP. Same as v14s13a_4, but Grayson mouth closed
    with dissolve

    u "What the fuck did you just say?"

    scene v14s13a_4
    with dissolve

    gr "Ha! Watch your tone, fruit cake. I was just joshing... With how defensive you got though, it does make me wonder a bit. *Chuckles*"

    scene v14s13a_4a
    with dissolve

    u "Where is this annoying joke coming from, Grayson?"

    scene v14s13a_4
    with dissolve

    gr "It's pretty well-known that Charli liked fucking with you. The fact that he was on vacation and still couldn't take his mind off of you..."

    scene v14s13a_4b # FPP. Same as v14s13a_4, different pose
    with dissolve

    gr "Maybe it's that beautiful sparkle in your eye... *Laughs*"

    scene v14s13a_4c # FPP. Same as v14s13a_4b, but Grayson mouth closed
    with dissolve

    u "I can't stand that fucker. You have no idea, man. He made my life a living-"

    scene v14s13a_4b
    with dissolve

    gr "Keep that anger built up, brother. We're gonna need you to have that energy in a few weeks."

    scene v14s13a_4c
    with dissolve

    u "For?"

    scene v14s13a_4
    with dissolve

    gr "There's a smell of... *Sniffs* change... in the air. And you need to be ready."

    scene v14s13a_4a
    with dissolve

    u "What exactly are you talking about?"

    scene v14s13a_4b
    with dissolve

    gr "All you need to know is that you should stay clean, be a good boy, and get into shape."
    
    scene v14s13a_4a
    with dissolve
    
    u "Are we fighting soon?"

    scene v14s13a_4
    with dissolve

    gr "Just be ready. Oh yeah, and welcome back. *Chuckles*"

    scene v14s13a_5 # TPP. Show Grayson leaving, closing the door behind him, smiling, mouth closed
    with dissolve

    u "(He will never make any sense to me.)"

    scene v14s13a_6 # TPP. Show MC removing his shirt (only in his boxers), slight smile, mouth closed
    with dissolve

    pause 0.75

    scene v14s13a_7 # TPP. Show MC turning the lights off
    with dissolve

    pause 0.75

    scene v14s13a_8 # TPP. Show MC plopping down on his bed
    with dissolve

    u "(Jet lag recovery begins in five... four... three... two...-)"

    scene v14s13a_9 # TPP. Show MC sleeping
    with dissolve

    pause 0.75

    jump v14s14a