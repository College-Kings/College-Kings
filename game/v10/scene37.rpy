label v10_riley_vs_amber:
    scene v10rva1 # First is their fingers interlaced while facing each
    with fade

    ri "Oh wow, you're kinda strong."

    play music "music/v10/Scene 37/Track Scene 37 & 38.mp3" fadein 2

    am "Kinda?"

    scene v10rva2 # second is Amber overpowering Riley and pushing her on her back
    with dissolve

    ri "*Scream* Ahh!"

    scene v10rva3 # third is Riley wrapping her legs around Amber's head
    with dissolve
    
    am "*Grunts*"
    
    aa "WOAH!"

    scene v10rva4 # fourth is Riley rolling Amber sitting on her face
    with dissolve
    
    ri "Not so strong now though."

    aa "Autumn, you think it'd be okay if I got in there with them?"

    aut "You can keep wishing. *Laughs*"

    scene v10rva5 # fifth is Amber sliding out
    with dissolve
    
    ri "Huh?"

    scene v10rva6 # sixth is Riley turning around and looking at Amber
    with dissolve
    
    am "That was kinda hot?"

    scene v10rva6a
    with dissolve
    
    ri "I'm well practiced. *Laughs*"

    scene v10rva7 # seventh is Amber kissing Riley
    with dissolve
    
    ri "*Muffled* Hmm?"

    u "(That's definitely something she'd do.)"
    
    scene v10rva8 # eight is Amber turning Riley around
    with dissolve

    ri "Woah!"

    scene v10rva9 # Amber holding Riley's arms behind her back
    with dissolve

    am "*Whisper* I like holding you like this..."

    scene v10rva9a
    with dissolve

    ri "Kinda hurts."

    scene v10rva9
    with dissolve

    am "You can always give up."

    scene v10rva9a
    with dissolve

    ri "I'm not-"

    scene v10rva9
    with dissolve

    am "*Grunts*"

    scene v10rva9b
    with dissolve

    ri "Alright!"

    scene v10rva9d
    with dissolve

    u "(Woah, that was... woah.)"

    scene v10rva10a # FPP. mouth closed
    with fade

    u "A kiss? Is that even fair? *Laughs*"

    scene v10rva10 # FPP. Show amber, smug smile, mouth open
    with dissolve

    am "It wasn't against the rules, plus I was in the mood for a kiss."

    scene v10rva10a
    with dissolve

    u "We were all wondering if you guys were gonna do a little more than kiss."

    scene v10rva10
    with dissolve

    am "*Chuckles* Maybe we will."

    scene v10rva10a
    with dissolve

    u "You're something else."

    scene v10rva11
    with dissolve

    aut "That was definitely exciting! I can tell my friend here enjoyed himself."

    scene v10rva12 # FPP. Show Aaron, slight, mouth open
    with dissolve

    aa "What was there not to enjoy? *Chuckles*"

    scene v10rva11
    with dissolve

    aut "Haha, very true."

    aut "Well I hope everyone enjoyed themselves as much as Aaron did."

    aut "That concludes this year's main event, though some of the stations will remain open a bit longer."

    aut "I want to thank everyone again for coming and for the donations."

    stop music fadeout 3
    jump v10_riley_walk

