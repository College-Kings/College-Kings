...
# Characters: MC (Outfit: 2), JENNY (Outfit: towel), CHLOE (Outfit: towel), AUBREY (Outfit: towel), NORA (Outfit: towel), LINDSEY (Outfit: towel)
# Time: Thursday


default v16s63_suggestion_box = False
default v16s63_mask = False
...
label v16s63: # 63) Chloe's Spa Day
    ...
    if relationship.jenny >= Relationship.FWB: # JUST A GUESS AT RELATIONSHIP, SINCE 'RS' MEANS SOMETHING DIFFERENT FOR EVERY GIRL # -if JennyRS
    ...
    if v16_masseuse: # PLACEHOLDER VARIABLE # -if MC is Masseuse
    ...
    if relationship.chloe == Relationship.GIRLFRIEND: # -if ChloeGf
        ...
        if relationship.nora >= Relationship.FWB: # VERIFY SINCE "RS" IS MEANINGLESS AND WRITING NEEDS TO STOP USING IT # -if NoraRS
        ...
        if relationship.jenny >= Relationship.FWB: # AGAIN, VERIFY. PLEASE STOP USING 'RS' # -if JennyRS
    ...
    if relationship.chloe >= Relationship.GIRLFRIEND: # -if chloeGF
        ...
        if (relationship.nora >= Relationship.FWB or relationship.aubrey >= Relationship.TAMED or relationship.jenny >= Relationship.FWB)
            scene v16s63_9d
            with dissolve
            ...
            if relationship.nora >= Relationship.FWB:
                scene v16s63_7
                with dissolve
            ...
            if relationship.aubrey >= Relationship.TAMED:
                scene v16s63_6a
                with dissolve
            ...
            if relationship.jenny >= Relationship.FWB:
    ...
    if v16s63_suggestion_box: # -if MC chose to write something
    ...
    if (v16s35_mc_spa_shopping == ??? or help_chloe = False): # PLACEHOLDER VARIABLES # -if MC bought the Expensive facemasks in Scene 35, or is not helping Chloe and was invited to help
        ...
        $ v14_lindsey_popularity += 2 # -LindseyPopularity +2
    ...
    scene v16s63_31 # FPP Show Aubrey (slight smile, mouth open) standing, looking back at the girls
    with dissolve

    pause 0.75


    if v16_masseuse: # -if MC is masseuse
        ...
        if v16s63_mask: # -if applied a mask
        ...
        if v16s63_mask: # -if applied a mask
            v16s63_33a # TPP Same angle as 33. MC (in facemask, mouth open) feeling the facemask
            with dissolve
